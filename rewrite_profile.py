...
content = content.replace("import { Link } from 'lucide-react';", "")
content = content.replace("Users,", "Users, Link, Copy, RefreshCw,")
...
content = content.replace("  const [loading, setLoading] = useState(true);", state_block)
...
# if the exact replace failed, try another hook
if ui_section not in content:
    content = content.replace("      </div>\n    );\n};", ui_section + "};\n")
